...
class UNet(nn.Module):
    def __init__(self, input_channels=3, output_channels=3, stages=[3, 3, 3], channels=[64, 128, 256], tanh=False, stem_size=4):
        super().__init__()
        self.encoder_first = nn.Conv2d(input_channels, channels[0], stem_size, stem_size, 0)

        self.decoder_last = nn.ConvTranspose2d(channels[0], output_channels*6, stem_size, stem_size, 0)
...
class DDPM(nn.Module):
    ...
    # time: Timestep
    def __init__(self, model=UNet(), num_timesteps=1000, beta_max=0.02, beta_min=10e-4):
        super().__init__()
        self.model = model
    ...
    def sample(self, x_shape=(1, 3, 64, 64), condition=None):
        ...
        with torch.no_grad():
            for t in tqdm(range(self.num_timesteps)):
                t = self.num_timesteps - t - 1
                t_scaled = t / self.num_timesteps
                beta = torch.linspace(self.beta_min, self.beta_max, steps=self.num_timesteps)
                alpha = 1 - beta
                alpha_bar_t = torch.prod(alpha[0:t]).item()
                next_t = t-1
                sigma_t = math.sqrt(1-torch.prod(alpha[0:next_t]))
                if t == 1:
                    sigma_t = 0
                x = (1/math.sqrt(alpha[t].item())) * x - ((1-alpha[t])/math.sqrt(alpha_bar_t) * self.model(x=x, time=t_scaled, condition=condition)) + torch.randn(*x.shape) * sigma_t
        return x
