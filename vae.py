...
class VectorQuantizer(nn.Module):
    def __init__(self, num_embeddings=8192, dim=16):
        super().__init__()
...
class VAE(nn.Module):
    ...
    def calclate_loss(self, x, noise_gain=0.1):
        z = self.encoder(x)
        z = z + torch.randn(z.shape, device=x.device) * noise_gain
...
class Encoder(nn.Module):
    def __init__(self, input_channels=3, latent_channels=16, channels=[64, 128, 256, 512], stages=[2, 2, 2, 2]):
        super().__init__()
